...
The result indicates success or failure, and includes error IDs if execution fails.
...
The `command_queue_client` node (provided by the `mg400_tools` package) is a ROS 2 node for sending a sequence of motion commands to the MG400 robot using the `command_queue` action interface.
...
- Can be used for scripting and batch execution of robot motions.

## Configuration

The client looks for a YAML file named `commands.yaml` in the following order:
1. `~/.config/mg400_tools/commands.yaml`
2. `<install_share>/mg400_tools/config/commands.yaml`
...
The node will automatically read the configuration, send the commands, and print feedback and results to the console.
